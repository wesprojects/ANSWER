...
(async () => {
  ...
  { const { info, hits } = await load((E, T) => { const n = E.addNode(T, 0, 0); E.addPanel(T, n, 0, 48, 54); }, 'thin');
    ...
    ck('1×48 thin: elevation length 4\'-0" nominal · 4\'-1" overall (1/2" end-of-run trim each end, p14)', /^4'-0" nominal · 4'-1" overall/.test(info.label) && near(info.totalW, 49, 1e-6), info.label);
    const posts = hits.filter(h => h.kind === 'post').sort((p, q) => p.x - q.x);
    ck('end-of-run trims just outside the nodes, 1/2" wide (p14)', posts.length === 2 && near(posts[0].x + 2, info.X0 - 0.5 * s) && near(posts[0].x + posts[0].w - 2, info.X0 + 0.75 * s) && near(posts[1].x + 2, info.X0 + 47.25 * s) && near(posts[1].x + posts[1].w - 2, info.X0 + 48.5 * s), JSON.stringify([posts.map(p => [p.x, p.w]), info.X0, s]));
    const cap = hits.find(h => h.kind === 'topcap'); ck('54" thin panel drawn 54 1/4" to the top of the cap (p11), cap 5/8"', near(cap.y + 3, y0 - 54.25 * s) && near(cap.h - 3, 0.625 * s), JSON.stringify([cap, y0, s]));
  ...
  { const { info, hits } = await load((E, T) => { const n = E.addNode(T, 0, 0); const a = E.addPanel(T, n, 0, 48, 54); const c = E.addPanel(T, T.nodes[a.b], 0, 48, 54); a.glassScreen = { attach: 'recessed', height: 12, frosted: false, omitGlass: false }; c.glassScreen = { attach: 'clip', height: 12, frosted: false, omitGlass: false }; }, 'thin');
    ...
    ck('recessed 12" glass drawn 9 5/16" high, 47 7/8" wide (p56)', near(g[0].h, 9.3125 * s) && near(g[0].w, 47.875 * s), JSON.stringify([g[0], s]));
    ck('clip glass drawn 11 3/4" high, 47 3/4" wide (p60)', near(g[1].h, 11.75 * s) && near(g[1].w, 47.75 * s), JSON.stringify([g[1], s]));
    ck('glass sits on the cap (54 1/4")', near(g[0].y + g[0].h, info.y0 - 54.25 * s), JSON.stringify([g[0], info.y0])); }
  // in-line change of height, glass on the lower panel: glass stops 1/2" from the junction center (47 7/16", p56)
  { const { info, hits } = await load((E, T) => { const n = E.addNode(T, 0, 0); const a = E.addPanel(T, n, 0, 48, 54); E.addPanel(T, T.nodes[a.b], 0, 48, 66); a.glassScreen = { attach: 'recessed', height: 12, frosted: false, omitGlass: false }; }, 'thin');
    const s = info.s, g = hits.find(h => h.kind === 'glass'); ck('glass next to change-of-height trim is 47 7/16" (p56)', near(g.w, 47.4375 * s), JSON.stringify([g, s]));
    const post = hits.filter(h => h.kind === 'post').sort((p, q) => p.x - q.x)[1]; ck('change-of-height trim rises to 66 19/32" (p11, p18)', near(post.y, info.y0 - 66.59375 * s), JSON.stringify([post, info.y0, s])); }
  ...
  { const { info, hits } = await load((E, T) => { const n = E.addNode(T, 0, 0); const a = E.addPanel(T, n, 0, 48, 54); E.addPanel(T, n, 270, 48, 54); }, 'thin', 0);
    ...
    // corner geometry (p15, p24): the junction block reaches 1 1/2" beyond the node and the corner face, where the skins stop, is 1 1/2" block + 3/4" post
    ...
    ck('corner junction 3 3/4" wide in the elevation: 1 1/2" block beyond the node + 2 1/4" to the corner face (p14, p15, p24)', corner && near(corner.w - 4, 3.75 * s), JSON.stringify([posts, info.nodeX, s]));
    ck('corner node to the far end node is 48" + 1 1/2" corner allowance (p15, p24)', near(info.nodeX[1] - info.nodeX[0], 49.5 * s, 1e-6), JSON.stringify(info.nodeX));
  ...
  { const { info, hits } = await load((E, T) => { const n = E.addNode(T, 0, 0); const a = E.addPanel(T, n, 0, 48, 54); a.topScreen = true; }, 'oval');
    const s = info.s; ck('1×48 oval: 4\'-2" overall (1" end-of-run trim each end, p76)', /4'-2" overall/.test(info.label), info.label);
    const g = hits.find(h => h.kind === 'glass'); ck('oval top screen 45 1/2" × 12" (p96)', g && near(g.w, 45.5 * s) && near(g.h, 12 * s), JSON.stringify([g, s]));
    const cap = hits.find(h => h.kind === 'topcap'); ck('54" oval panel drawn 54 1/8" (p74)', near(cap.y + 3, info.y0 - 54.125 * s), JSON.stringify([cap, info.y0, s])); }
