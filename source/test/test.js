// Engine tests against the spec guide's worked examples (p38-39) and rule checks.
...
// and ordered once for the job as packages (rollupPackages, p374-375). The guide examples print one light-seal package per junction; the
// junction lines now count seals (2 for a T, 4 for an X, p34 Step 4), so those expectations read 2 or 4. Aligner counts were already pieces.
// Example One (p38): X, four heights. A 66 and B 54 adjacent, C 42 opposite A, D 30 opposite B.
{
  const P = E.newProject('thin'); const c = star(P, { 0: 66, 90: 54, 180: 42, 270: 30 });
  const res = E.generate(P);
  expect('p38 Example One — X 4 heights BYO', nodeLines(P, res, c), { TS766JP: 1, TS754JP: 1, TS742JP: 1, TS730JP: 1, TS790JB4: 1, TS766ICLS: 4, TS712TICHT: 1, TS712TCLCHT: 1, TS712TCTCHT: 1, TS790COHJC: 1, TS7LTA4: 1, TS7CJCA10: 1 });
}
// Example Two (p38): X 54,54,42,30 (A,B tallest adjacent)
{
  const P = E.newProject('thin'); const c = star(P, { 0: 54, 90: 54, 180: 42, 270: 30 });
  const res = E.generate(P);
  expect('p38 Example Two — X 3 heights BYO', nodeLines(P, res, c), { TS754JP: 2, TS742JP: 1, TS730JP: 1, TS790JB4: 1, TS766ICLS: 4, TS712TCLCHT: 1, TS712TCTCHT: 1, TS790JC: 1, TS7LTA4: 1, TS7CJCA10: 2 });
  console.log('  light seal 66" as printed in the guide example (never below 66", never below the tallest junction)');
}
// Example Three (p38): Y 54/42/30
{
  const P = E.newProject('thin'); const c = star(P, { 0: 54, 120: 42, 240: 30 });
  const res = E.generate(P);
  expect('p38 Example Three — Y 3 heights BYO', nodeLines(P, res, c), { TS754JP: 1, TS742JP: 1, TS730JP: 1, TS7120JB3: 1, TS712T120CHT: 1, TS712TVCHT: 1, TS7120JC: 1, TS7120VA4: 1 });
}
// p35 T Option 1: A 78, C 66 (spine), B 30 perpendicular -> EOR 12 (78-66), T CoH 36 (66-30), T vertical 30, COHJC, aligner
{
  const P = E.newProject('thin'); const c = star(P, { 0: 78, 180: 66, 90: 30 });
  const res = E.generate(P);
  expect('p35 T Option 1 — 3 heights', nodeLines(P, res, c), { TS778JP: 1, TS766JP: 1, TS730JP: 1, TS790JB3: 1, TS778ICLS: 2, TS712TICHT: 1, TS736TCTCHT: 1, TS766TTVT: 1, TS790COHJC: 1, TS7CJCA10: 1 });
}
// p35 T Option 2: A 66 tall, B (perp) 54, C 30 -> EOR 12, L CoH 24, T vertical 30, COHJC, L-to-T aligner
{
  const P = E.newProject('thin'); const c = star(P, { 0: 66, 90: 54, 180: 30 });
  const res = E.generate(P);
  expect('p35 T Option 2 — 3 heights', nodeLines(P, res, c), { TS766JP: 1, TS754JP: 1, TS730JP: 1, TS790JB3: 1, TS766ICLS: 2, TS712TICHT: 1, TS724TCLCHT: 1, TS730TTVT: 1, TS790COHJC: 1, TS7LTA4: 1, TS7CJCA10: 1 });
}
// p37 X Option 8: A 66 > C 54 > B 42, D 30 -> EOR 12 (A-C), T CoH 12 (C-B), T CoH 24 (C-D), COHJC, aligner x1, blocks 4
{
  const P = E.newProject('thin'); const c = star(P, { 0: 66, 180: 54, 90: 42, 270: 30 });
  const res = E.generate(P);
  expect('p37 X Option 8 — 4 heights', nodeLines(P, res, c), { TS766JP: 1, TS754JP: 1, TS742JP: 1, TS730JP: 1, TS790JB4: 1, TS766ICLS: 4, TS712TICHT: 1, TS712TCTCHT: 1, TS724TCTCHT: 1, TS790COHJC: 1, TS7CJCA10: 1 });
}
...
// Panel with segmented skins + window and open base -> frame package + skins, F suffix rule not applicable with open base? (skins to the floor)
{
  ...
  const res = E.generate(P);
  const pl = res.lines.filter(l => l.src === p.id);
  expect('36x66 buildup with window, skins to floor, power', pl, { TS736THF: 1, TS72436TKF: 2, TS71236HS: 2, TS72436SPW: 1, TS7PK36X: 1, TS71SSX: 1, TS72SSX: 1, TS73SSX: 1, TS76BPX: 1, TS7UFPLATE: 3 }); // faceplates: receptacles cut through skins to the floor (p107, p167)
...
// ---------- power and stability regressions (p107, p158, p163-166, p380, p390, p458, p480, p124-127, p1) ----------
{
  ...
  // #1 infeed occupies one receptacle location (p158, p480)
  ...
  // #2 base infeed needs a base-location modular powerkit behind base trim (p158)
  ...
  // #3 faceplates for receptacles in field-cut fabric skins (p107, p167, p479)
  ...
  // #4 no opening for receptacles (p050, p051, p108, p163)
  ...
  // #5 hardwire: frame package with omitted trims + hardwire base trims + hardwired powerkit, no modular power (p166, p380, p482)
  ...
  // #6 non-PVC power is not a panel-package option (p390) -> frame package + non-PVC kit
  ...
  // #7 extra-length harness: pass-through at a different height, open-base bottom kit at a corner (p164)
  ...
  // #8 technology skins resolve from UI cutout values; receptacles required in every power block location (p163, p458)
  ...
  // #9 return narrower than 30" does not anchor a run over 8' (p126)
  const run = (widths, left, right, extra) => { const P = E.newProject('thin'); let n = E.addNode(P, 0, 0); const s = n; const nodes = []; for (const w of widths) { const p = E.addPanel(P, n, 0, w, 54); n = P.nodes[p.b]; nodes.push(n); } if (left) E.addPanel(P, s, 90, left, 54); if (right) E.addPanel(P, n, 90, right, 54); if (extra) extra(P, nodes); return E.generate(P).warnings.filter(w => /\(p12[67]\)/.test(w.msg)); };
  ...
  // #10 over 18': 48"W perpendicular every 12', measured through T junctions (p127)
  ...
  // #11 both ends of a run over 8' are anchored; a free end within 8' of a return is fine (p126 figures)
  ...
  // #12 Canadian list: each base price and option x1.09 rounded, then added (p1)
...
// p27/p362: one stacking junction per stacker; the 36" end-of-run stacker is never an end-of-run stacking junction
{
  ...
  expect('EOR 42 + 24/12 stackers -> 24" + 12" EOR stacking junctions (p362)', nl, { TS742TEPJ: 1, TS724TEPJS: 1, TS712TEPJS: 1, TS778TEVT: 1 });
...
// p41 Step 7: spine stackers over a thin T base = one EOR stacking junction per panel + junction blocks (p42 Step 9); L pair shares an L
{
  ...
  expect('p41 Step 7 left — T 42, spine stacked 12', nodeLines(P, E.generate(P), c), { TS742TTPJ: 1, TS712TEPJS: 2, TS754TTVT: 1, TS712TCTCHT: 1, TS790JC: 1, TS7CJCA10: 2, TS766ICLS: 2, TS790JB3: 1 });
  ...
  expect('p41 Step 7 right — T 42, adjacent pair stacked 12', nodeLines(P2, E.generate(P2), c2), { TS742TTPJ: 1, TS712TLPJS: 1, TS742TTVT: 1, TS712TCLCHT: 1, TS790JC: 1, TS7CJCA10: 2, TS7LTA4: 1, TS766ICLS: 2 });
...
// p349/p19: build-your-own in-line change-of-height needs the stacking horizontal frame package
{
...
// p352-353: T change-of-height is handed only when A and C differ
{
...
// p14: no 30" wall-start junction -> flagged line, never an undefined style
{
  ...
  check('wall-start 30" flagged, no undefined style', nl.length === 1 && nl[0].style === '—' && nl[0].flags.length === 1 && res.warnings.some(w => /Wall-start/.test(w.msg)), nl.map(l => l.style).join());
}
// p20/p349: in-line change-of-height to a panel stacked to 90" uses the 54/90, 66/90, 78/90 rows
{
  ...
  // The p349 tip's stacking horizontal frame package is the taller panel's own stacking-tier package here (p42 Step 8: stacking junctions on
...
// p28/p374: light seals top out at 78"; a taller junction is flagged
{
...
// p374: junction block packages of 3/4/5 mixed for the cheapest exact count (6 = 3+3, not 2 x 5)
{
...
// ---- oval change-of-height trims, stacking trim finishes, skins/windows/glass rule checks (audit fixes) ----
{
  ...
  // p81: 36"H standard CoH is the tallest; above it use stacking CoH trim
  expect('oval L 30/78 CoH: standard 36 + stacking 12 (p81)', trims(ovalStar({ 0: 30, 90: 78 })), { TS736CHS: 1, TS712CHST: 1 });
  // p81: more than one stacked panel -> standard on the first tier, stacking on the second
  expect('oval in-line 42 / 42+12+12 CoH: standard 12 + stacking 12 (p81)', trims(ovalStar({ 0: 42, 180: 42 }, [null, [12, 12]])), { TS712CHS: 1, TS712CHST: 1 });
  expect('oval in-line 42 / 42+24 CoH: one standard 24 (p81)', trims(ovalStar({ 0: 42, 180: 42 }, [null, [24]])), { TS724CHS: 1 });
  expect('oval L 42 / 54+24 CoH: one standard 36 (p81)', trims(ovalStar({ 0: 42, 90: 54 }, [null, [24]])), { TS736CHS: 1 });
  // p418: wood stacking CoH trim is its own style; fabric is an option on steel trim
  const wood = (P) => { P.finishes.woodTrim = true; P.finishes.ovalWoodTrim = true; }, fab = (P) => { P.finishes.ovalTrimFabric = true; };
  expect('oval L 30/78 wood trim: TS736CHSW + TS712CHSTW (p417-418)', trims(ovalStar({ 0: 30, 90: 78 }, null, wood)), { TS736CHSW: 1, TS712CHSTW: 1 });
  { const st = trims(ovalStar({ 0: 30, 90: 78 }, null, fab)).find(l => l.style === 'TS712CHST'); check('oval stacking CoH fabric trim option (p418)', st && /fabric trim/.test(st.spec) && st.unit === 56 + 52, st && st.spec + ' $' + st.unit); }
  // p406: stacking L/T junction trim in wood is the ...PJSW style; fabric +$80 on steel trim
  { const r = ovalStar({ 0: 54, 90: 54 }, [[12], [12]], wood); const sj = nodeLines(r.P, r.res, r.c).filter(l => l.cat === 'Stacking'); check('oval wood trim: stacking L junction is TS712LPJSW (p406)', sj.length === 1 && sj[0].style === 'TS712LPJSW' && sj[0].unit === 427, sj.map(l => l.style + ' $' + l.unit).join(',')); }
  { const r = ovalStar({ 0: 54, 90: 54 }, [[12], [12]], fab); const sj = nodeLines(r.P, r.res, r.c).filter(l => l.cat === 'Stacking'); check('oval fabric trim: stacking L junction +$80 fabric (p406)', sj.length === 1 && sj[0].style === 'TS712LPJS' && sj[0].unit === 396 && /fabric trim/.test(sj[0].spec), sj.map(l => l.style + ' $' + l.unit + ' ' + l.spec).join(',')); }
  // single-panel helper
  ...
  // p117: technology covers, one per cutout opening (cutouts given as the planner's UI values)
  ...
  check('technology covers: 48"W All = 2, Right = 1, None = 0; 72"W All = 4; 24"W All = 1 (p117)', tech('All') === 2 && tech('Right') === 1 && tech('None') === 0 && tech('All', 72) === 4 && tech('All', 24) === 1, [tech('All'), tech('Right'), tech('None'), tech('All', 72), tech('All', 24)].join('/'));
  // p436: 48"H and 60"H to-the-floor fabric skins are vertical only
  { const r = one('thin', 66, p => { p.skinsToFloor = true; }); const f = r.pl.filter(l => /TKF$/.test(l.style)); check('60"H to-the-floor fabric skins get vertical application (p436)', f.length === 2 && f.every(l => /vertical application/.test(l.spec)), f.map(l => l.spec).join(' | ')); }
  // p118 / p12: window placement and pairing
  const win = (h) => ({ kind: 'window', height: h, pane: 'single' }), sk = (h) => ({ kind: 'skin', type: TA, height: h });
  { const r = one('thin', 54, p => { p.sides = [[win(24), sk(24)], [win(24), sk(24)]]; }); check('window at the base of a panel warns (p118)', r.wm.some(m => /base of a panel/.test(m)), r.wm.join(' | ')); }
  { const r = one('thin', 66, p => { p.sides = [[sk(24), win(12), sk(24)], [sk(24), win(12), sk(24)]]; }); check('window below the top of a base panel warns (p51, p118)', r.wm.some(m => /top position only/.test(m)), r.wm.join(' | ')); }
  { const r = one('thin', 66, p => { p.sides = [[sk(36), win(24)], [sk(60)]]; }); check('window on one side only warns (p12)', r.wm.some(m => /no matching window/.test(m)), r.wm.join(' | ')); }
  ...
  { const r = one('thin', 48, (p, P) => { p.sides = [[sk(18), win(24)], [sk(18), win(24)]]; E.setStack(P, p, [12, 12]); p.stackSides = [0, 1].map(() => [[win(12)], [win(12)]]); });
    check('3 stacked windows and windows on a 24" top window warn (p118)', r.wm.some(m => /No more than two/.test(m)) && r.wm.some(m => /24"H glass window in the top/.test(m)), r.wm.join(' | '));
    check('both stack tiers windows: one stacking frame package kept (p55, p119)', r.pl.filter(l => /HFS$/.test(l.style)).length === 1, r.pl.filter(l => /HFS$/.test(l.style)).map(l => l.style).join(',')); }
  // p113: slatwall not in the bottom 12"; brace package only when asked for
  { const r = one('thin', 42, p => { p.sides[0] = [{ kind: 'skin', type: 'slatwall', height: 12 }, sk(24)]; }); check('slatwall at the bottom warns, no brace by default (p113)', r.wm.some(m => /bottom 12/.test(m)) && !r.pl.some(l => /SBP$/.test(l.style)), r.wm.join(' | ')); }
  { const r = one('thin', 42, p => { p.sides[0] = [sk(24), { kind: 'skin', type: 'slatwall', height: 12, brace: true }]; }); check('slatwall with brace flag orders the brace package (p113)', r.pl.some(l => l.style === 'TS71248SBP') && !r.wm.some(m => /bottom 12/.test(m)), r.pl.map(l => l.style).join(',')); }
  // p57 / p386: recessed glass top cap connector
  { const P = E.newProject('thin'); star(P, { 0: 54, 90: 54 }); for (const q of Object.values(P.panels)) q.glassScreen = { attach: 'recessed', height: 12 }; const res = E.generate(P); check('no recessed top cap connector at a same-height corner (p57)', !res.lines.some(l => l.style === 'TS7TFGRC')); }
  { const P = E.newProject('thin'); star(P, { 0: 54, 90: 66 }); const low = Object.values(P.panels).find(q => q.height === 54); low.glassScreen = { attach: 'recessed', height: 12 }; const res = E.generate(P); check('recessed top cap connector at a corner with a taller leg (p57)', res.lines.filter(l => l.style === 'TS7TFGRC').reduce((a, l) => a + l.qty, 0) === 1); }
  { const P = E.newProject('thin'); star(P, { 0: 54, 180: 66 }); const low = Object.values(P.panels).find(q => q.height === 54); low.glassScreen = { attach: 'recessed', height: 12 }; const res = E.generate(P); check('recessed top cap connector at in-line change-of-height, glass on the lower panel (p386)', res.lines.filter(l => l.style === 'TS7TFGRC').reduce((a, l) => a + l.qty, 0) === 1); }
  { const r = one('thin', 54, (p, P, a) => { a.wallStart = true; p.glassScreen = { attach: 'recessed', height: 12 }; }); check('recessed top cap connector at a wall start (p57)', r.pl.filter(l => l.style === 'TS7TFGRC').reduce((x, l) => x + l.qty, 0) === 1); }
  // p444-449: laminate and wood skins need the trim paint color
  { const r = one('thin', 42, p => { p.sides = [[{ kind: 'skin', type: 'laminate', height: 36 }], [{ kind: 'skin', type: 'wood', height: 36 }]]; }); const ls = r.pl.filter(l => /LS$|WS$/.test(l.style)); check('laminate and wood skins specify trim paint (p444, p446)', ls.length === 2 && ls.every(l => /trim paint 7207/.test(l.spec)), ls.map(l => l.style + ' ' + l.spec).join(' | ')); }
}
// ---- oval stacking junctions: same type as the base junction (p77), start at the shared tallest junction (p81, p84), one per stacker tier ----
{
  ...
  expect('oval in-line 42 / 42+24: in-line stacking junction + standard CoH, no EOR stacker (p77, p81)', ov({ 0: 42, 180: 42 }, [null, [24]]), { TS742IPJ: 1, TS724IPJS: 1, TS724CHS: 1 });
  expect('oval L 42 / 54+24: L stacking junction from 54" + standard CoH 36 (p77, p81)', ov({ 0: 42, 90: 54 }, [null, [24]]), { TS754LPJ: 1, TS724LPJS: 1, TS736CHS: 1 });
  // one change-of-height trim run per exposed face (p80): legs 0 and 270 expose 42"→66" (standard 12 on the first stacked tier + stacking 12
  // on the second, p81), leg 90 exposes 54"→66" (standard 12). Earlier the band model counted one trim per height band, not per face.
  expect('oval X 42 / 42+12 / 42+24 / 42: X stacking junctions only (p77), CoH trim per exposed face (p80)', ov({ 0: 42, 90: 42, 180: 42, 270: 42 }, [null, [12], [24], null]), { TS742XPJ: 1, TS712XPJS: 2, TS712CHS: 3, TS712CHST: 2 });
  expect('oval in-line 66 / 42+24: no stacking junction below the shared 66" junction (p81, p84)', ov({ 0: 66, 180: 42 }, [null, [24]]), { TS766IPJ: 1 });
  expect('oval in-line 54 / 42+24: in-line stacker 54"→66" only + 12 CoH (p81)', ov({ 0: 54, 180: 42 }, [null, [24]]), { TS754IPJ: 1, TS712IPJS: 1, TS712CHS: 1 });
  { const P = E.newProject('oval'); const a = E.addNode(P, 0, 0); const p = E.addPanel(P, a, 0, 48, 42); E.setStack(P, p, [24, 12]); const res = E.generate(P);
    expect('oval end-of-run 42+[24,12]: TS724EPJS + TS712EPJS (p86, p407)', res.lines.filter(l => l.src === a.id), { TS742EPJ: 1, TS724EPJS: 1, TS712EPJS: 1 }); }
  expect('oval in-line both [18,18]: two 18" in-line stackers (p86, p405)', ov({ 0: 42, 180: 42 }, [[18, 18], [18, 18]]), { TS742IPJ: 1, TS718IPJS: 2 });
  expect('oval wood trim L 54 +12: wood base + TS712LPJSW (p397, p406)', ov({ 0: 54, 90: 54 }, [[12], [12]], P => { P.finishes.woodTrim = true; P.finishes.ovalWoodTrim = true; }), { TS754LPJW: 1, TS712LPJSW: 1 });
  { const ls = ov({ 0: 54, 120: 54 }, [[12], null], P => { P.finishes.ovalTrimFabric = true; }); const v = ls.find(l => l.style === 'TS712VPJS');
    const ok = v && v.unit === 399 + 80 && /fabric trim/.test(v.spec) && ls.some(l => l.style === 'TS712CHS');
    console.log((ok ? 'PASS ' : 'FAIL ') + 'oval fabric trim V 54 / 54+12: V stacking junction +$80 fabric, CoH on the unstacked face (p408): ' + ls.map(l => l.style + ' $' + l.unit).join(', ')); if (!ok) fails++; }
}
// ---------- workstations: supports (p207-217), worksurfaces (p506-526), pedestals (p608-614) ----------
{
  ...
  // 1. 18"D corners: nominal 18 (not 19), BOM finds the style, P and SW edges offered (p521)
  { const z = E.cornerSizes('corner', 'cord-drop').find(z => z.style === 'UCC114242'); const { P, c, a, b } = L2(42, 42); const w = E.newWorksurface(P, { kind: 'corner', node: c.id, legs: [a.id, b.id], C: z.C, D: z.D, depthA: z.depthA, depthB: z.depthB }); const res = E.generate(P);
    expect('18"D corner from the size list -> UCC114242 (p521)', wsLines(res), { UCC114242: 1, UCANT: 2, USSBR: 1 }); check('18"D corner nominal depth and edges (p521)', z.depthA === 18 && JSON.stringify(E.wsEdgesFor(P, w)) === '["3mm","P","SW"]', `depth ${z.depthA}, edges ${E.wsEdgesFor(P, w)}`); }
  // 2. extended corner arms over 54" take a reinforcing channel (p524 tip, p208, p546)
  ...
  // 3. a fixed pedestal under a 30"D corner arm is 30"D (p609, p273)
  { const { P, c, a, b } = L2(48, 48); const w = E.newWorksurface(P, { kind: 'corner', node: c.id, legs: [a.id, b.id], C: 48, D: 48, depthA: 30, depthB: 30 }); E.addPedestal(P, w, { at: 'arm0', type: 'fixed' }); expect('30"D corner + fixed pedestal -> RPF3027AF with its filler (p273, p610)', wsLines(E.generate(P)), { UCC334848: 1, UCANT: 1, RPF3027AF: 1, RPXFTAKFP: 1, USSBR: 1 }); }
  // 4. a seam where the lower worksurface ends on a pedestal: the next worksurface carries its own cantilever (p217)
  ...
  // 5. two 30"D in line: end panels go on the free ends, the seam keeps one shared cantilever (p207, p217)
  ...
  // 6. two corners meeting at one junction share one cantilever (p217, p545 tips)
  ...
  // 7. extended corner hand follows the plan: C and A are the user's left arm (p524)
  ...
  // 8. on-module supports sit on panel junctions (p216); corner arms 1/2" less than the panel width (p209)
  ...
  // 9. long worksurfaces: a cantilever at the junction under them keeps supports within 54" (p208, p217)
  ...
  { const { P, ps } = run([72]); E.newWorksurface(P, { panel: ps[0].id, width: 72, depth: 24 }); expect('72"W on one 72" panel -> TS7WKSPT72 (p208)', wsLines(E.generate(P)), { US2472: 1, UCANT: 2, TS7WKSPT72: 1 }); }
  // 10-11. pulls are priced per pull; c:scape is proud steel only (p608, p613)
  { const { P, ps } = run([48]); const w = E.newWorksurface(P, { panel: ps[0].id, width: 48, depth: 24 }); E.addPedestal(P, w, { at: 'hi', type: 'fixed', config: 'A', front: 'P', pull: 'jazz' }); E.addPedestal(P, w, { at: 'lo', type: 'mobile', config: 'B', front: 'W', pull: 'c:scape' }); const res = E.generate(P);
    const bbf = res.lines.find(l => l.style === 'RPF2427AP'); check('jazz pulls on a box/box/file: +$17 × 3 (p608)', bbf && bbf.unit === 658 + 51, bbf && '$' + bbf.unit);
    check('c:scape on a proud wood front -> error (p613)', wsErr(res).some(e => /c:scape/.test(e.msg))); }
  // 12. no 18"D end panel (p547)
  ...
  // edge not offered: no invented style number (p507 N.A.)
...
// ---- worksurface floor space: worksurfaces butt at the junctions, nothing overlaps (p209) ----
{
  ...
  { const { P, c, a, a2, b } = L(); const k = E.newWorksurface(P, { kind: 'corner', node: c.id, legs: [a.id, b.id], C: 42, D: 42, depthA: 24, depthB: 24 }); const g = E.wsGeometry(P, k); const endAlong = g.arms.map(x => Math.round(Math.hypot(x.end[0], x.end[1]) * 100) / 100);
    ...
    // corner geometry (p15, p24, p209, p521): the 42" junction is 42" + 1 1/2" corner allowance from the corner node; the 41 1/2" cord-drop arm back edge
  ...
  { const { P, a, b } = L(); const sA = E.sideNormal(P, a, 0)[1] < 0 ? 0 : 1, sB = E.sideNormal(P, b, 0)[0] > 0 ? 0 : 1;
    // off -1.5: both start at the corner node, 1 1/2" short of the module line where a straight wrapped by the return fits (corner geometry, p15, p24)
...
// ---- tie plates at butted worksurface seams (p217, p546) ----
{
  const ck = (name, ok, info) => { console.log((ok ? 'PASS ' : 'FAIL ') + name + (info ? ' — ' + info : '')); if (!ok) fails++; };
  ...
  { const { P, A, B } = two(); for (const w of [A, B]) { E.addPedestal(P, w, { at: 'lo', type: 'fixed', config: 'A' }); E.addPedestal(P, w, { at: 'hi', type: 'fixed', config: 'A' }); } const R = E.generate(P); const tp = R.lines.filter(l => l.pid === 'uw-tie-plates');
    ck('seam over pedestals, no cantilevers: TS7TIEPLATE package ordered (p546)', tp.length === 1 && tp[0].style === 'TS7TIEPLATE' && tp[0].qty === 1, tp.map(l => l.qty + 'x' + l.style + ' ' + l.desc).join()); }
}
// ---- p35-p37 build-your-own options: the heights used by the planner's guide-example presets reproduce each option's printed trim list ----
{
  ...
  const cases = [
    ['p35 T Option 1', [[0, 66], [90, 42], [180, 54]], { TICHT: 1, TCTCHT: 1, TTVT: 1, COHJC: 1, CJCA10: 1 }],
    ['p35 T Option 2', [[0, 66], [90, 54], [180, 42]], { TICHT: 1, TCLCHT: 1, TTVT: 1, COHJC: 1, LTA4: 1, CJCA10: 1 }],
    ['p35 T Option 3', [[0, 54], [90, 66], [180, 42]], { TICHT: 1, TCLCHT: 1, TTVT: 1, COHJC: 1, LTA4: 1, CJCA10: 1 }],
    ['p35 Y Option 1', [[0, 66], [120, 54], [240, 42]], { T120CHT: 1, TVCHT: 1, JC120: 1, '120VA4': 1 }],
    ['p36 X Option 1', [[0, 54], [90, 66], [180, 42], [270, 66]], { TCTCHT: 2, JC: 1, CJCA10: 2 }],
    ['p36 X Option 2', [[0, 42], [90, 54], [180, 66], [270, 66]], { TCLCHT: 1, TCTCHT: 1, JC: 1, CJCA10: 2, LTA4: 1 }],
    ['p36 X Option 3', [[0, 42], [90, 54], [180, 54], [270, 66]], { TICHT: 1, TCTCHT: 1, COHJC: 1, CJCA10: 2 }],
    ['p36 X Option 4', [[0, 42], [90, 54], [180, 66], [270, 54]], { TICHT: 1, TCTCHT: 1, COHJC: 1, CJCA10: 2 }],
    ['p36 X Option 5', [[0, 42], [90, 42], [180, 54], [270, 66]], { TICHT: 1, TCLCHT: 1, COHJC: 1, CJCA10: 1 }],
    ['p36 X Option 6', [[0, 42], [90, 54], [180, 42], [270, 66]], { TICHT: 1, TCTCHT: 2, COHJC: 1, CJCA10: 1 }],
    ['p37 X Option 7', [[0, 30], [90, 42], [180, 54], [270, 66]], { TICHT: 1, TCLCHT: 1, TCTCHT: 1, COHJC: 1, CJCA10: 1, LTA4: 1 }],
    ['p37 X Option 8', [[0, 66], [90, 42], [180, 54], [270, 30]], { TICHT: 1, TCTCHT: 2, COHJC: 1, CJCA10: 1 }],
  ];
...
// ---- dealer spec review regressions (items 1-8) ----
{
  ...
  ck('#1 splitSpan 42 trims -> 30+12, 30 stack -> 18+12, 6 impossible', JSON.stringify(E.splitSpan(42, E.COH_TRIM_HEIGHTS).parts) === '[30,12]' && JSON.stringify(E.splitSpan(30, E.STACK_HEIGHTS).parts) === '[18,12]' && E.splitSpan(6, E.STACK_HEIGHTS).flag && !E.splitSpan(42, E.STACK_HEIGHTS).flag);
  { const P = E.newProject('thin'); const c = E.addNode(P, 0, 0); E.addPanel(P, c, 0, 48, 48); E.setStack(P, E.addPanel(P, c, 90, 48, 66), [24]); const R = E.generate(P); const nl = nodeLines(P, R, c.id);
    ck('#1 thin L 48 | 66+24: 42" end-of-run CoH as 30+12 trims (p43, p370), no placeholder', cnt(nl, 'TS730TICHT') === 1 && cnt(nl, 'TS712TICHT') === 1 && !nl.some(l => l.style === '—') && !R.errors.length, sty(nl)); }
  { const P = E.newProject('oval'); const c = E.addNode(P, 0, 0); E.addPanel(P, c, 0, 48, 48); E.setStack(P, E.addPanel(P, c, 90, 48, 66), [24]); const R = E.generate(P); const nl = nodeLines(P, R, c.id);
    ck('#1 oval L 48 | 66+24: standard 30 + stacking 12 (p81, p417-418)', cnt(nl, 'TS730CHS') === 1 && cnt(nl, 'TS712CHST') === 1 && !nl.some(l => l.style === '—'), sty(nl)); }
  { const P = E.newProject('thin'); const c = star(P, { 0: 48, 180: 78 }); const R = E.generate(P); const nl = nodeLines(P, R, c);
    ck('#1 BYO in-line 48/78: TS748TIPJ + 18" and 12" end-of-run stackers (p41 Step 6, p362)', cnt(nl, 'TS748TIPJ') === 1 && cnt(nl, 'TS718TEPJS') === 1 && cnt(nl, 'TS712TEPJS') === 1 && cnt(nl, 'TS730TICHT') === 1 && !R.errors.length, sty(nl)); }
  { const P = E.newProject('thin'); const c = E.addNode(P, 0, 0); E.setStack(P, E.addPanel(P, c, 0, 48, 42), [24]); E.addPanel(P, c, 90, 48, 54); const R = E.generate(P); const nl = nodeLines(P, R, c.id);
    ck('#1 thin L 42+24 beside 54: one 24" stacking junction (p41 Step 7, per stacked panel), not 12+12 cut at the neighbor top', cnt(nl, 'TS724TEPJS') === 1 && !cnt(nl, 'TS712TEPJS') && !nl.some(l => l.style === '—') && !R.errors.length, sty(nl)); }
  // #2 oval: one change-of-height trim per exposed face (p80)
  ...
  { const P = E.newProject('thin'); const sp = []; let n = E.addNode(P, 0, 0); for (let i = 0; i < 3; i++) { const p = E.addPanel(P, n, 0, 48, 42); E.setStack(P, p, [24]); sp.push(p); n = P.nodes[p.b]; }
    ...
    ck('#3 aligners: pieces per junction at $0, one TS7CJCA10 package ordered for the job', pieces.length >= 2 && pieces.every(l => l.ext === 0) && pack.length === 1 && pack[0].qty === 1 && pack[0].ext === 72 && pack[0].src === 'project', `${pieces.map(l => l.src + ':' + l.qty).join(',')} / ${pack.map(l => l.qty + '@' + l.unit).join()}`);
  ...
  // #4 72"W+ single-pane window over a fabric or steel skin: two T521328SR clips (p119, p464)
  ...
  // #5 stacking horizontal frame package with an in-line change-of-height: shared, not one per junction (p349 tip, p42 Step 8)
  ...
  // #6 technology skin must face the powerkit (p117, p163)
  ...
  { const P = E.newProject('thin'); star(P, { 0: 48, 180: 54 }); const R = E.generate(P);
    ck('#7 in-line 48/54 (6"): error, no priced placeholder', R.errors.some(e => /cannot be built/.test(e.msg)) && !R.lines.some(l => l.style === '—' && l.qty > 0), R.errors.map(e => e.msg).join(' | ').slice(0, 140));
    const P2 = E.newProject('thin'); star(P2, { 0: 48, 90: 54, 180: 30 }); const R2 = E.generate(P2);
    ck('#7 T 48/54/30 BYO: 6" CoH trim impossible -> error', R2.errors.some(e => /cannot be trimmed/.test(e.msg)), R2.errors.map(e => e.msg).join(' | ').slice(0, 140)); }
  // #8 wall-start anchor named in the stability warning (p137)
...
// ---- outputs regressions: SIF code-less options, stable line keys, oval cap colors, worksurface edge, CAD per line, aggregate by source ----
{
  ...
  say('p401: oval Y junction specifies the plastic cap color', /^plastic junction cap 6000 Black$/.test(Y.spec), Y.spec);
  const eor = R.lines.find(l => /End-of-run base junction/.test(l.desc));
  say('p397/p399: oval end-of-run specifies trim paint and cap color', /^paint 7207 Black; paint group 1; plastic junction cap 6000 Black$/.test(eor.spec), eor.spec);
  ...
  say('p401: wood cap Y junction specifies the wood', Yw.style === 'TS748YPJW' && /^wood junction cap 3062 Graphite Walnut$/.test(Yw.spec), Yw.style + ' ' + Yw.spec);
  ...
  say('Canadian list per line adds up to the job total (p1)', R.lines.reduce((a, l) => a + E.cadOf(l), 0) === R.totals.canadian && E.cadTotal(R.lines) === R.totals.canadian);
  ...
  say('p716: worksurface edge is its own setting (6009 with 2730), not the receptacle color', /edge plastic 6009 Arctic White/.test(ws.spec) && E.edges().some(e => e.code === '6655') && E.ovalPlastics().some(o => o.code === '6694'), ws.spec);
}
// ---- plan geometry: panels may only meet at junctions (p14-15); stacking trims to what fits; trim switches keep settings ----
{
  const ck = (name, ok, info) => { console.log((ok ? 'PASS ' : 'FAIL ') + name + (info ? ' — ' + info : '')); if (!ok) fails++; };
  ...
  // corner geometry: the X's 24" leg ends 24" + 1 1/2" corner allowance out (p15, p24, E.CORNER_ALLOW), so the return starts at 25.5
  ...
  // setStack keeps the largest part of the stack that fits under 90" (p27)
  ...
  { const P = E.newProject('thin'); const p = run(P, 0, 0, 0, [48])[0]; p.glassScreen = { attach: 'recessed', height: 12, frosted: false, omitGlass: false }; p.sides[0] = [{ kind: 'skin', type: 'tackable acoustical', height: 36 }, { kind: 'window', height: 12, pane: 'single' }]; p.sides[1] = JSON.parse(JSON.stringify(p.sides[0])); const R = E.generate(P);
    ck('window in the top position removes the glass screen with a notice (p57)', !p.glassScreen && R.notices.some(m => /cannot sit over a window/.test(m)), R.notices.join()); }
}
// ---- workstation QA round 2 (items 1-19): seams, wraps, fits, geometry, pedestals, pulls ----
{
  ...
  // U back wall: 96" between 36" returns. A 48" wrapped at the left leaves no room for a 48" on the right; a 42" butted 3" away is not a seam (p209, p216)
  ...
  // #4 side support bracket only on a return panel that matches the worksurface depth (p216)
  // off 0: the panel's module starts 1 1/2" out from the L node (corner geometry, p15, p24), at the return panel's face; this was off 1.5 when modules started on the node
  ...
  // #6 extended corner: straight front on the long arm's extra length, cove only at the corner (p524)
  { const P = E.newProject('thin'); const c = E.addNode(P, 0, 0); const a = E.addPanel(P, c, 0, 60, 54); const b = E.addPanel(P, c, 90, 42, 54); const w = E.newWorksurface(P, { kind: 'extcorner', node: c.id, legs: [a.id, b.id], C: 60, D: 42, depthA: 24, depthB: 24 }); const g = E.wsGeometry(P, w);
    ...
    // the short arm's actual back edge is 41 1/2" (catalog widthC, cord drop, p524), measured from the rear corner; it was 42" - 2" when arms ended on the nominal width
  ...
  // #7 c:scape pulls are painted 4140/4144/4799 (p274)
  ...
  // #9 36"D (35 1/2") straights are freestanding only (p508 tip): a panel-mounted one is an error, and the planner does not offer the depth on panels
  { const { P, ps } = run([72]); const w = E.newWorksurface(P, { panel: ps[0].id, width: 72, depth: 36 }); const R = E.generate(P);
    ck('#9 panel-mounted 72×36 -> still listed as US3672, error: freestanding only (p508)', styles(R, w.id).includes('US3672') && errsOf(R, w.id).some(m => /freestanding/.test(m.msg || m)), JSON.stringify([styles(R, w.id), R.errors])); }
  ck('#9 36"D is not offered on panels', !E.WS_DEPTHS.includes(36));
  // #10 center support panel at a free end (p217)
  ...
  // #12 L of two straight worksurfaces (p209 tip, p226): the return butts the front edge; full depth for even gaps
  // off 0 (was 1.5): the modules of panels at the L node start 1 1/2" out from it, at the return panel's face (corner geometry, p15, p24)
  ...
  { const { P } = LL('cord-drop'); const R = E.generate(P); ck('#12 cord-drop L -> uneven-gap warning (p209 tip)', R.warnings.some(w => /uneven gaps/.test(w.msg)) && !R.errors.length, JSON.stringify(R.errors)); }
  { const { P } = LL('cord-drop', 'P'); const R = E.generate(P); ck('#11 P-edge in an L -> valley warning and interference (p209)', R.warnings.some(w => /valley/.test(w.msg)) && R.errors.some(e => /overlaps/.test(e.msg))); }
  ...
  // #13 a 30"D straight beside a corner arm still needs front-edge floor support (p207)
  ...
  // #14 a junction more than 54" from the end still takes a cantilever (p208)
  ...
  // #15 a junction under a long extended-corner arm takes a cantilever before a channel (p208)
  ...
  // #18 the 1/2" cord-drop gap is behind the worksurface (p208); fronts line up with full-depth worksurfaces
...
// final QA round: workstations in a pod, power runs, collision wording, oval wood trim spec, side A/B wording
{
  ...
  // oval wood trim: the trim is wood, so no trim paint on those lines (p399)
...
// ---- physical dimensions used by the drawings (dimension audit, Feb 2015 guide) ----
{
  ...
  const r = E.junctionReach('thin', 'EOR'); ck('dims: thin end-of-run post 3/4" inside the module, trim node..+1/2" (p14, p39)', eq(r.post[0], -0.75) && eq(r.post[1], 0) && eq(r.trim[0], 0) && eq(r.trim[1], 0.5));
  ck('dims: oval end-of-run trim +1" (p76)', eq(E.junctionReach('oval', 'EOR').trim[1], 1));
  // corner geometry (p15, p24): the 3" corner block reaches 1 1/2" beyond the node and the corner face is 1 1/2" block + 3/4" post = 2 1/4" toward the panel
  ck('dims: in-line junction 1 1/2" centered (p24), corner block 3" (p14) and corner face 2 1/4" (p15, p24)', eq(E.junctionReach('thin', 'inline').post[1] - E.junctionReach('thin', 'inline').post[0], 1.5) && eq(E.junctionReach('thin', 'L').out, 1.5) && eq(E.junctionReach('thin', 'L').in, 2.25) && eq(E.junctionReach('thin', 'L').ca, 1.5));
  ck('dims: thin wall-start face 3/16" beyond the node (p15)', eq(E.junctionReach('thin', 'wall').out, 0.1875) && eq(E.junctionReach('thin', 'wall').post[1], 0.1875));
  ck('dims: actual heights 54 -> 54 1/4" thin, 54 1/8" oval, 30 -> 29 1/2" (p11, p74)', eq(E.actualBaseHeight('thin', 54), 54.25) && eq(E.actualBaseHeight('oval', 54), 54.125) && eq(E.actualBaseHeight('thin', 30), 29.5));
  ck('dims: thin cap 5/8" = panel 54 1/4" - end-of-run trim 53 5/8" (p11, p30)', eq(E.actualBaseHeight('thin', 54) - E.CAP_FACE.thin, 53.625) && eq(E.actualBaseHeight('thin', 42) - E.CAP_FACE.thin, 41.25));
  { const P = E.newProject('thin'); const c = E.addNode(P, 0, 0); const p = E.addPanel(P, c, 0, 54, 54); E.setStack(P, p, [12]); ck('dims: 54 + 12 stacker drawn 54 1/4 + 12 3/8 (p11, p26)', eq(E.actualTop('thin', p), 66.625), E.actualTop('thin', p)); }
  ck('dims: recessed glass 9 5/16 / 15 1/2 / 21 5/8, clip 11 3/4 (p56, p60)', eq(E.glassHeight({ attach: 'recessed', height: 12 }), 9.3125) && eq(E.glassHeight({ attach: 'recessed', height: 18 }), 15.5) && eq(E.glassHeight({ attach: 'recessed', height: 24 }), 21.625) && eq(E.glassHeight({ attach: 'clip', height: 12 }), 11.75));
  ck('dims: glass 47 7/8 recessed, 47 3/4 clip, 47 7/16 / 47 1/4 with a change-of-height end on a 48 (p56, p60)', eq(48 - 2 * E.GLASS.recessed.end, 47.875) && eq(48 - 2 * E.GLASS.clip.end, 47.75) && eq(48 - E.GLASS.recessed.end - E.GLASS.recessed.cohEnd, 47.4375) && eq(48 - E.GLASS.clip.end - E.GLASS.clip.cohEnd, 47.25));
  ck('dims: oval top screen 45 1/2" on a 48 (p96)', eq(48 - 2 * E.TOP_SCREEN.inset, 45.5) && eq(E.TOP_SCREEN.height, 12));
  { const P = E.newProject('oval'); ck('dims: oval change-of-height trim 1 1/8" slim, 2 1/4" cable routing (p80)', eq(E.cohTrimWidth(P), 1.125) && (P.options.ovalCohProfile = 'Cable-Routing Capability', eq(E.cohTrimWidth(P), 2.25))); }
  ck('dims: open base 3 1/4" with 2 1/2" opening (p51), worksurface 1 3/16" thick (p206)', eq(E.OPEN_BASE.height, 3.25) && eq(E.OPEN_BASE.opening, 2.5) && eq(E.WS_THICK, 1.1875));
  ...
  // worksurface run limit at a wall start is the wall face (p15)
  ...
  // the technology-skin power check starts skins on the 3 3/4" base trim (p50)
...
// ---------- corner junction geometry (p15 block-and-post, p24 46 1/2" frame, p208 1/2" cord drop, p209 butted worksurfaces, p521/p526 corner sizes) ----------
// A corner node stays on the centerline intersection; each panel's module starts E.CORNER_ALLOW out from it (1 1/2" at L, T, X; 2/sqrt(3) - 1/2" at V, Y).
{
  ...
  { const { P, c, a, a2, b, b2 } = L();
    ...
    ck('L of 48s: frame 46 1/2" between the corner face (2 1/4") and the in-line junction face (p24)', near(E.junctionReach('thin', 'L').in, 2.25) && near(fr, 46.5), fr);
